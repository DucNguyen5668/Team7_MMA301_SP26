...
export default function PostScreen() {
  ...
  const handleAIGenerate = async () => {
    if (images.length === 0) {
      Alert.alert(
        "Chưa có ảnh",
        "Vui lòng chọn ít nhất 1 ảnh để AI có thể nhìn thấy sản phẩm nhé!",
      );
      return;
    }
    ...
    try {
      ...
      if (desc) {
        ...
        Alert.alert(
          "Lỗi AI",
          "AI không thể sinh văn bản lúc này. Vui lòng thử lại.",
        );
      }
    } catch (error) {
  ...
  const handleGetLocation = async () => {
    try {
      ...
      if (status !== "granted") {
        Alert.alert(
          "Thiếu quyền",
          "Vui lòng cấp quyền định vị GPS để tự động lấy địa chỉ.",
        );
        return;
      }
  ...
  const handlePost = async () => {
    if (!title || !price || category === "Danh mục") {
      Alert.alert(
        "Thiếu thông tin",
        "Vui lòng nhập điền Tiêu đề, Giá và Danh mục cho sản phẩm.",
      );
      return;
    }
      ...
      >
        ...
        <View style={[styles.row, { marginBottom: 16 }]}>
          <TouchableOpacity
            style={[
              styles.condBtn,
              condition === "used" && styles.condBtnActive,
            ]}
          ...
          >
            <Text
              style={[
                styles.condBtnText,
                condition === "used" && styles.condBtnTextActive,
              ]}
            >
              Đã sử dụng
            </Text>
          ...
          <TouchableOpacity
            style={[
              styles.condBtn,
              condition === "new" && styles.condBtnActive,
            ]}
          ...
          >
            <Text
              style={[
                styles.condBtnText,
                condition === "new" && styles.condBtnTextActive,
              ]}
            >
              Mới
            </Text>
